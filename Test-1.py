...
colors = {
    ...
    # more colors more fun, dont forgor the highlight
}
def select_image():
...
process_button = tk.Button(left_frame, text="Process Image", command=lambda: process_image(selected_color.get(), highlight_mode.get(), color_tolerance_scale.get(), segmentation_threshold_scale.get(), [slider.get() for slider in lower_bound_sliders], [slider.get() for slider in upper_bound_sliders], kernel_size_scale.get()))
process_button.pack()
